...
barchart(key ~ freq, data = head(stats, 30), 
         col = "cadetblue", main = "Ingredientes por receita", xlab = "Frequência")
...
Com a função abaixo fizemos um crawler com objetivo de verificar quais das 100 primeiras receitas estão no site [epicurious](https://www.epicurious.com). A função `epicurious` recebe o nome da receitas (`title`) e retorna o link da receitas no site, caso ela se encontre nele, e
...
epicurious <- function(recipe_title){
  ...
  for(i in seq_along(recipe_title)){
    print(i)
    #try(if(str_length(recipe_title[i]) < 35) stop('Insert a valid title!') )
...
```{r}
stats <- txt_freq(unlist(recipes_top100[recipes_in_epicurious$inside_epicurious,
                                        ]$categories))
stats$key <- factor(stats$key, levels = rev(stats$key))
stats
